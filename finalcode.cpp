...
int valid_numInput(int min, int max, string& prompt) {
    ...
    while (true) {
        ...
        if (cin.fail() || value < min || value > max) {
            cout << "\n\033[1mERROR!! Please enter a valid number between " << min << " and " << max <<"\033[0m"<< endl;
...
char valid_charInput(string& prompt, char opt1, char opt2) {
    ...
    while (true) {
        ...
        if (value != opt1 && value != opt2 || cin.fail()) {
            cout << "\n\033[1mERROR!! Please enter a valid option i.e, " << opt1 << " or " << opt2 <<"\033[0m"<< endl;
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        } else return value;
    }
...
string valid_strInput(string& prompt, string validOpt[], int optCount) {
    ...
    while (true) {
        ...
        for (int i = 0; i < optCount; i++) {
            if (value == validOpt[i])           //compares user input value with each string in the array
                return value;
        }
        cout << "\n\033[1mERROR!! Please enter a valid option:\033[0m";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
}
...
void ProgressBar() {
    cout << "\n\033[1mAnalyzing your car's condition:\033[0m\n";
...
void userInput(int& carYear, int& tireChanges, int& rechargeFrequency, int& exteriorChoice, string& exteriorCondition, int& fuelEfficiency, char& accidentHistory, char& servicedRegularly) {
    ...
    string carYear_prompt = "Enter car model year: ";
    string fuelEfficiency_prompt = "Enter car average (fuel efficiency in km/L): ";
    string tireChanges_prompt = "Enter the number of tire changes: ";
    string rechargeFrequency_prompt = "How often do you recharge the battery?\n[1] Every few days\n[2] Weekly\n[3] Monthly\n[4] Rarely\n \nEnter your choice: ";
    string accidentHistory_prompt = "Has the car experienced any accidents? (y/n): ";
    string servicedRegularly_prompt = "Do you service the car regularly? (y/n): ";
    string exteriorChoice_prompt = "What is the exterior condition of the car?\n[1] Rusted\n[2] Scratched\n[3] Excellent\n\nEnter your choice (1-3): ";
    cout << "\n----------------------------------------\n" << endl;
    ...
    cout << "\n----------------------------------------\n" << endl;
    fuelEfficiency = valid_numInput(1, 30, fuelEfficiency_prompt);              //input feul efficiency with error validation
    cout << "\n----------------------------------------\n" << endl;
    tireChanges = valid_numInput(0, 10, tireChanges_prompt);                    //input tire changes with error validation
    cout << "\n----------------------------------------\n" << endl;         
    rechargeFrequency = valid_numInput(1, 4, rechargeFrequency_prompt);         //input recharge frequency with error validation
    cout << "\n----------------------------------------\n" << endl;
    accidentHistory = valid_charInput(accidentHistory_prompt, 'y', 'n');        //input accident history with error validation
    cout << "\n----------------------------------------\n" << endl;
    servicedRegularly = valid_charInput(servicedRegularly_prompt, 'y', 'n');    //input service frequency with error validation
    cout << "\n----------------------------------------\n" << endl;
    exteriorChoice = valid_numInput(1, 3, exteriorChoice_prompt);                //input exterior condition choice with error validation
    cout << "\n----------------------------------------\n" << endl;
    // assigning each exterior condition choice a string variable
    if (exteriorChoice == 1)
...
int calculateRating(int& carYear, int& tireChanges, int& rechargeFrequency, int& exteriorChoice, string& exteriorCondition, int& fuelEfficiency, char& accidentHistory, char& servicedRegularly, int rating) {
    ...
    // Evaluate accident history and service frequency
...
void displayResults(int rating) {
    ...
    cout << "\nYour car rating: ";                  // Displaying car rating result with color coding
    ...
    if (suggest == 'y') {
        ...
        if (rating < 89) {
            if (carYear < 2020) {
                cout << "\033[95m- Upgrade your car model for a better rating.\033[0m\n";
                cout<<endl;
            }
            if (servicedRegularly == 'n') {
                cout << "\033[95m- Regularly service your car.\n";
                cout << endl;
            }
            if (rechargeFrequency == 3 || rechargeFrequency == 4) {
                cout << "\033[95m- Recharge the battery more frequently.\033[0m\n";
                cout << endl;
            }
            if (fuelEfficiency < 10) {
                cout << "\033[95m- Improve fuel efficiency by maintaining the engine and driving efficiently.\033[0m\n";
                cout << endl;
            }
            if (exteriorCondition == "rusted" || exteriorCondition == "scratched") {
                cout << "\033[95m- Address exterior issues like rust or scratches.\033[0m\n";
                cout << endl;
            }
            if (tireChanges > 3) {
                cout << "\033[95m- Replace tires if worn.\033[0m\n";
                cout << endl;
            }
        } else {
            cout << "\033[95m- Your car is in excellent condition! Keep maintaining it.\033[0m\n";
            cout << endl;
        }
    }
...
void locationServices(string& province) {
    ...
    string validprovs[] = { "Punjab", "punjab", "Sindh", "sindh", "Balochistan", "balochistan", "KPK", "kpk" };         //array to store the set of strings
    ...
    if (province == "Punjab" || province == "punjab") {
        ...
        cout << "3. Tahir Motor Workshop (31.42890627002953, 73.09110428249656)\033[0m" << endl;
    } else if (province == "Sindh" || province == "sindh") {
        cout << "\033[3m1. Usama Automotive Workshop (24.907252079432062, 67.06093424686144)" << endl;
        ...
        cout << "\033[3m1. Rahuf Ustad Workshop (30.33650477629199, 68.5750062756085)" << endl;
    ...
    } else if (province == "KPK" || province == "kpk") {
        cout << "\033[3m1. Hazara Autos Workshop (34.04167865544165, 72.94499144919025)" << endl;
...
int menu(int menuChoice) {
    menuChoice = 0;
    cout << "\nWelcome to the ultimate Car Assistant where we can render a free online service for your car !!! What adventure awaits you today?" << endl;
    string menuChoice_prompt = "\n1. Virtual Car Analysis - Lets see how your car stacks up \n2. Location Services - Workshops near you \n3. Exit - Come back soon :( \nPlease enter your choice: ";
...
int main() {
    while (true) {
        ...
        char choice;
          cout << "\n----------------------------------------\n" << endl;
        string choice_prompt = "Do you want to exit? (y/n): ";
